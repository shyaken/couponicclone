...
class UWorklet extends CBaseController
{
	...
	public function __call($name,$parameters)
	{
		...
		if(method_exists($this,'before'.$name))
			if(($before = call_user_func_array(array($this,'before'.$name),$parameters))!==null)
				return $before;
		// worklet task
		$result = method_exists($this,'task'.$name)
			? call_user_func_array(array($this,'task'.$name),$parameters)
			: parent::__call('task'.$name,$parameters);
		// worklet after			
	...
	{		
		...
		app()->cache->set($this->getCacheKeyPrefix($varyByBehaviors) . $name,
			$value,$duration,$dependency);
	}
	...
	{
		return isset($this->module->params[$name])
	...
	protected function accessDenied($user=null,$message=null)
	{
		...
		if($user->getIsGuest())
			...
			throw new CHttpException(403,
				$message!==null?$message:Yii::t('yii','You are not authorized to perform this action.'));
	}
