...
class UStorageBin extends CComponent
{
	...
	public function getFilePath($name)
	{
		$file = $this->get($name);
		return $file
			? Yii::getPathOfAlias($this->getPath()) . DS . $file->hash . '.' . $file->extension
			: null;
	}
	...
	public function getFileUrl($name,$schema='')
	{
		$file = $this->get($name);
		return $file
			? UHelper::pathToUrl($this->getPath()) . '/' . $file->hash . '.' . $file->extension
			: null;
	}
	...
	public function delete($name, $entry=true)
	{
		...
		if($filePath = $this->getFilePath($name))
		{
			...
			return $entry && ($file = $this->get($name))!==null
				? $file->delete()
				: true;
		}
		return false;
