...
class WInstallHelper extends USystemWorklet
{
	...
	public function taskLoadModules()
	{
		...
		$this->resetModules(app(),$modules);
	}
	
	public function taskResetModules($parent,$modules)
	{
		...
		foreach($parent->modules as $id=>$cfg)
		{
			if(isset($cfg['modules']))
			{
				...
				if($m)
					$this->resetModules($m,$cfg['modules']);
			}
		}
	}
	...
	public function taskPrepare($type,$id)
	{
		if($id == 'app')
			foreach(app()->getAppModules() as $m)
				$this->prepare($type,$m);

		$module = $id!='app' ? UFactory::getModuleFromAlias($id) : app();				
		if($type == 'upgrade' && !$module->params['version'])
			wm()->get('install.confirm')->addModule($module);		
	}
	...
	public function taskInstaller($type,$id,$version=null,$autoInstall=false,$requiredBy=null)
	{
		...
		if($id!='app')
			$module = UFactory::getModuleFromAlias($id);
		else
			$module = app();
		...
		{			
			...
			else
			{
				...
				if(version_compare($module->param('version'), $module->getVersion()) >= 0)
					return;
