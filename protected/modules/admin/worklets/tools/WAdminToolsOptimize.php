...
{	
	...
	{
		$this->collectData(Yii::getPathOfAlias('uniprogy.framework'),'js', array(
			'jquery-1.4.2.min.js',
			'jquery.scrollTo.js'
		));
		$this->collect(app());
	...
	public function collectData($folder,$type,$excludes=array())
	{
		$excludes[] = '.';
		$excludes[] = '..';
		...
		{		
			...
			{
				if(!in_array($file,$excludes) && is_file($folder.'/'.$file))
				{
